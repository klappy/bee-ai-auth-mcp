...
it('approved grant still respects the runtime window before any Container work', async () => {
  calls.status.mockResolvedValue({ status: 'approved', epoch: 1 });
  const response = await McpApiHandler.fetch(request, env(), { props: { login: 'person@example.test', beeToken: 'synthetic', admissionEpoch: 1 } } as any);
  expect(response.status).toBe(503); expect(calls.reserve).toHaveBeenCalledOnce(); expect(calls.container).not.toHaveBeenCalled(); expect(calls.handler).not.toHaveBeenCalled();
});
