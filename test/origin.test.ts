...
const SELF = "https://beeauth.example.dev/mcp";
...
describe("isOriginAllowed", () => {
  ...
  it("passes same-origin requests", () => {
    expect(isOriginAllowed("https://beeauth.example.dev", SELF)).toBe(true);
  });
  ...
  it("rejects scheme-downgrade even on the same host", () => {
    expect(isOriginAllowed("http://beeauth.example.dev", SELF)).toBe(false);
  });
