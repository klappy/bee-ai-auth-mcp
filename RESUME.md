...
`main` is current (E0020). **Phase 1 — auth core + per-grant encrypted custody + the private-CA Container bridge — is LIVE and phone-validated** at `bee.klappy.dev`: `whoami` returns the operator's Bee identity end-to-end through the bridge (E0019). **Phase 2 — the read surface — is BUILT and merged to `main` (E0020):** two tools, **`bee_docs`** (serves `docs/bee-api-usage.md`) and **`bee_read`** (read-only retrieval — GET any `/v1/*`, POST only to the allow-listed `/v1/search/*`; `/v1/stream` + all mutations refused; 512KB cap). `bee_read` generalizes the validated `whoami` transport (bound bridge + per-grant `beeToken`). It is **typecheck-clean and now fresh-context validated — functionally sound and safe on `main` (E0021 / D0037), accept-with-named-residuals.**

**⮕ NEXT ACTION: close the operator-only residuals** the validator could not exercise from a validator seat (E0021 H1–H3): (1) demonstrate single-tenant denial — connect as a second, non-allow-listed GitHub login and confirm the 403 "Not authorized"; (2) infra-layer no-token-in-logs audit — confirm the Cloudflare Worker request logs and the bridge container logs (`observability.enabled`) carry no `Authorization` header; (3) prior-art PII pass over `planning/`. All three code paths are verified — these are the runtime/infra checks only the operator can run. Latest journals: `odd/ledger/2026-06-16-phase2-read-surface-fresh-validation.md` (E0021 — the validation) and `odd/ledger/2026-06-16-phase2-read-surface-built-and-merged.md` (E0020). Authoritative spec: `PRD.md` (v0.5). Deploy runbook: `bridge/README.md`. Merge-to-`main` is a code milestone, not the validated DoD — do not promote to prod before the fresh-context green.

**Deploy model:** a `git push` auto-deploys via Cloudflare's connected Git integration (Workers Builds) — branch push = preview (`npx wrangler versions upload` to `https://<slug>-bee-ai-auth-mcp.klappy.workers.dev`), merge/commit to `production` = prod at `bee.klappy.dev`. No manual `wrangler`, no CF token. See `docs/ci-cd.md`.
...
- **Mode: Phase-2 read surface VALIDATED (E0021) — functionally sound + safe on `main`, accept-with-named-residuals (D0037).** Built + merged (E0020); Phase-1 wire proof validated (E0019). Remaining = operator-only residuals (E0021 H1–H3: second-login-denial demo, infra-layer no-token-in-logs audit, prior-art PII pass over `planning/`). PRD is **v0.5** (Phase 2 read surface specced).
...
2. **Validate the read surface — DONE (E0021).** Live black-box across the full read surface + both search modes, white-box (typecheck + 15 unit tests + source audit), mapped to the gate's 5-corroboration. Verdict **D0037: functionally sound + safe on `main`, accept-with-named-residuals.** Operator-only residuals (H1–H3) remain.
...
- `odd/ledger/2026-06-16-phase2-read-surface-fresh-validation.md` — **E0021: fresh-context validation (verdict D0037, accept-with-residuals); companion `docs/phase2-read-surface-fresh-validator-review.md`.**
