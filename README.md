...
5. **Deploy by pushing** — commit/merge to `production` deploys to prod at `bee.klappy.dev` via Cloudflare Workers Builds (which also builds the bridge container image); a branch push is a preview at `https://<slug>-bee-ai-auth-mcp.klappy.workers.dev` via `npx wrangler versions upload`. No manual `wrangler deploy`. `main` is the integration branch; the operator promotes by merging to `production` (commit title: "Promote main to production").
