...
Use [the hosted connection guide](https://bee.klappy.dev/#connect) for Claude, ChatGPT or Grok, with `https://bee.klappy.dev/mcp`. The draft path is: Klappy approves your email, you sign in with an inbox code, then approve your own Bee using the link or QR generated by the hosted Bee CLI. Invitees need no GitHub or Cloudflare account, hosting, or local CLI. **Release hold:** this path depends on auth PR34 being released and verified, including Cloudflare config and hosted E2E; the homepage draft must not publish before that. Existing GitHub login remains an optional path. The hosted service handles your Bee credential and retrieved data; see [privacy and access](https://bee.klappy.dev/security).
...
Either way: approve in the Bee app. The hosted invitee draft has the service run Bee's official CLI, present that CLI's link or QR, then finish the connection into your encrypted grant. Invitees do not install or run the CLI. No keychain spelunking or token paste is required. Both variants also show a copyable **connect URL** for manual entry into the Bee app's "Enter Bee ID" field, and the raw-token paste box remains further below as the ultimate fallback.

Two things worth knowing:

- **The Bee app may label the approval Bee CLI.** On the hosted invitee draft (pending auth PR34), the service runs Bee's official CLI as a one-shot broker. This draft is not evidence the hosted CLI broker is live. Current self-host pairing on this branch still uses the Worker handshake in `src/pairing.ts`.
...
**Connecting your Bee.** Use the consent screen's phone deep link or desktop QR and approve in the Bee app. The hosted draft path uses the CLI-generated link or QR from the hosted Bee CLI; you do not install the CLI locally. A manual token path remains a fallback; see `docs/connecting-and-getting-your-bee-token.md`.
