...
| **Phase 2 — Retrieval egress** | Read-only MCP tools: `bee_docs` (serves the Bee-API-usage doc) + a single `bee_read` method-split passthrough — **GET** any `/v1/*`, **POST** only to the allow-listed `/v1/search/*` (Bee search is POST but non-mutating); `/v1/stream` + all mutations refused; size-capped. Read-only by construction; `bee_write` deferred to the write phase. Docs-first | Bee retrieval usable from claude.ai across surfaces — **built + merged (E0020); runtime validation pending** |
