...
- The first-use path clearly identifies invited friends and family, Klappy's GitHub approval, the user's own Bee account, the shared https://bee.klappy.dev/mcp endpoint, app-specific setup, Bee approval, and a first retrieval prompt.
...
- The homepage explicitly describes operator custody, encrypted grants, processing through the host, AI-provider receipt, and the limits of disconnecting. It neither asks users to reveal credentials in chat nor claims that hosted operation eliminates trust in Klappy.
