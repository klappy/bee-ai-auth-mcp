...
2. **deployed-validation** — PR only. Live smoke against `vars.DEPLOYED_VALIDATION_URL` when that repo variable names an **isolated** Worker. If the variable is unset, the job records the named gap and succeeds. It does **not** poll `https://<slug>-bee-ai-auth-mcp.klappy.workers.dev`. It does **not** smoke `https://bee.klappy.dev` from a PR. It does **not** change the non-production build command into `wrangler deploy`.
