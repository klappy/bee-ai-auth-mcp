...
1. **check** — `npm ci` → `npm run typecheck` → `npm test`. Pure units, no network. **This is the required automated gate.** Optional `npm run test:image` probes a locally built `bee-bridge:local` (`scripts/probe-bridge-image.sh`). Optional `npm run test:handshake` runs compiled `broker start` on a synthetic id and keeps only sanitized status/shape/exit. Missing docker/image exits 2 (named skip). Neither is a required CI job and neither creates a paid Worker.
