...
Local browser evidence (2026-09-09, source-equivalent static `public/` on `127.0.0.1`, not hosted auth E2E): desktop hero/plans, Copy URL success status “Copied. Paste this URL into your AI app.”, `#connect`/`#plans`/`#help` anchors, Claude/ChatGPT/Grok details, keyboard focus, and a ~400px narrow layout were observed. Raw `python -m http.server` 404s `/security` and `/setup` because it does not map extensionless HTML; a session-only pretty-URL mapper on `:4174` returned 200 for `/`, `/security`, `/setup`, and `/roadmap`. Denied-clipboard fallback was not re-run in the browser this pass; the fallback status string remains in `public/index.html`. Authenticated pairing, inbox OTP, Bee-app approval, and live MCP retrieval were not performed.
