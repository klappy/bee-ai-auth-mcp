...
Not claimed live: required secret presence, Access application/policy configuration, real browser email and direct GitHub flows, Bee pairing, and MCP end-to-end behavior remain release holds. Preview URLs are not a supported validation surface for this Durable Object + Container Worker; the CI contract is now `docs/ci-cd.md` and `docs/ci-validation-debrief.md`. Public-facing copy remains draft pending approval.
